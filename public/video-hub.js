...
function render(filter='all'){grid.innerHTML='';const list=records.filter(v=>filter==='all'||v.type===filter||v.categories.includes(filter));if(!list.length){grid.innerHTML='<div class="loading-card">No videos found in this category yet.</div>';return}list.forEach(v=>{const card=document.createElement('article');card.className='yt-card';card.innerHTML='<a href="/videos/watch/'+v.id+'" class="yt-thumb"><img loading="lazy" src="'+thumb(v.id)+'" alt="'+v.title.replace(/"/g,'&quot;')+'"><span class="yt-badge">'+(v.type==='shorts'?'SHORT':'VIDEO')+'</span><span class="yt-play">▶</span></a><div class="yt-body"><h3>'+v.title+'</h3><p>'+labelFor(v.categories[0])+' · <a class="text-link" href="/videos/watch/'+v.id+'">Read the GpSpace guide →</a></p></div>';card.querySelector('.yt-thumb').addEventListener('click',e=>{e.preventDefault();setFeatured(v)});grid.appendChild(card)})}
